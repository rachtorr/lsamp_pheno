...
In the Google Colab or Rmd file, you will see emojis to indicate a prompt to either stop and think and answer a question (🧠✍️) or to change a line of code and rerun it (💻). The goal is to help think through the steps and consider what the output means. 
...
🧠✍️ Based on the output, consider the following questions:
...
💻 Try rerunning the same code as above but with `geom_line()`.
...
🧠✍️ Run the code below and think about:
...
💻 Try changing from `doy` to `month` in the above code (lines 9-15 in Google Colab, or 210-217 in R)
...
💻 Note: as of June 2026, NEON requires an API token to download data.Set up your API token [here](https://www.neonscience.org/resources/learning-hub/tutorials/api-token-setup). This should be a string that you save on your machine, then include this additional parameter to the `loadByProduct()` function: `token="yourAPIkey"`

In this case I have saved my NEON API key in my `.Renviron` file, and then I read it in with `Sys.getenv("NEON_API_KEY")`. See an example of doing this [here](https://laurenilano.com/posts/api-keys/).
...
🧠✍️ What patterns are you noticing? Does there look like there's a relationship between temperature and greenness?
...
🧠✍️ What patterns are you noticing? Does GCC90 or RCC90 appear to have a stronger relationship with minimum daily temperature?
