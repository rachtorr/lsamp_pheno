...
wd <- "~/GitHub/lsamp_pheno/data/" # Change this to match your local environment
setwd(wd)
...
phenophaseOfInterest <- c("Leaves","Open flowers")
...
phenoPlot <- ggplot(inStat_T, aes(dateStat, countYes, fill=phenophaseName), position="dodge") +
...
phenoPlot <- ggplot(inStat_T, aes(dateStat, countYes, fill=phenophaseName)) +
    geom_bar(stat="identity", na.rm = TRUE, position="dodge") +
    ggtitle("Total Individuals in Leaf or Flower out") +
...
phenoPlot_P <- ggplot(inStat_T, aes(dateStat, percent, fill=phenophaseName)) +
    geom_bar(stat="identity", na.rm = TRUE, position="dodge") +
    ggtitle("Proportion in Leaf of Flower out") +
...
# Now let's make the plot look a bit more presentable
phenoPlot18 <- ggplot(phe_1sp_2018, aes(dateStat, countYes, fill=phenophaseName)) +
    geom_bar(stat="identity", na.rm = TRUE, position="dodge") +
