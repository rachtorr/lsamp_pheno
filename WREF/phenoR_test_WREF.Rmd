...
my_site = "NEON.D16.WREF.DP1.00033"
vegtype="EN"
...
phenocamr::download_phenocam(
  veg_type = vegtype, #DB for decid broadleaf, EN for evergreen, default is ALL
  roi_id = 1000,
  site = my_site,
  phenophase = TRUE,
...
df = read_phenocam(paste(my_site,vegtype,"1000_3day.csv", sep="_"))
...
dft = read_phenocam(paste(my_site,vegtype, "1000_3day_transition_dates.csv", sep="_"))$data
...
```{r daymet}
site_clim = merge_daymet(paste(my_site, vegtype, "1000_3day.csv", sep="_"))
...
site_avg = site_data %>% group_by(doy) %>% summarize(gdd = mean(gdd))

ggplot(site_avg, aes(x=doy, y=gdd)) + 
  geom_point() + ggtitle("growing degree day")
...
pheno_data = phenocam_EN$windriver  
