...
phenocamr::download_phenocam(
 ...
 # veg_type = "DB", #DB for decid broadleaf, EN for evergreen, default is ALL
  ...
  site = name,
  phenophase = TRUE,
  out_dir = "../data/" # saves in current working dir
  )
...
data_path = paste("../data/", name, "_WL_1000_3day.csv",sep="")

# read as phenocamr type 
df = read_phenocam(data_path)
...
# plot rcc90
ggplot(bnh_df, aes(x=as.Date(date), y=rcc_90)) + geom_point() +geom_smooth()
...
dft = read_phenocam("../data/esyampah_WL_1001_3day_transition_dates.csv")$data
...
bnh_clim = merge_daymet(data_path)
...
# start with one variable 
lm_test = lm(data=data_mod, rcc_90~dayl..s.)
# see residuals, p-vals, Rsquares, etc. 
summary(lm_test)

# check with more variables 
lm_test2 = lm(data=data_mod, rcc_90~dayl..s.+tmin..deg.c.)

# predict with remaining years
data_pred = bnh_data %>% 
  dplyr::filter(year>=2014) %>% 
  select(rcc_90, dayl..s., tmin..deg.c., year, doy, date) 

# predict with one var
data_pred$pred1_rcc_90 = predict.lm(lm_test, data_pred)

# predict with 2 vars 
data_pred$pred2_rcc_90 = predict.lm(lm_test2, data_pred)

ggplot(data_pred) + 
  geom_point(aes(x=date, y=pred1_rcc_90, col="1 var")) + 
  geom_point(aes(x=date, y=pred2_rcc_90, col="2 vars"))+ 
  geom_point(aes(x=date, y=rcc_90, col="obs")) 
