...
# filter by site id
grsm_site <- phenology_targets %>% filter(site_id=="GRSM")
...
getVarsEddy("filesToStack00200/NEON.D07.GRSM.DP4.00200.001.nsae.2024-06.basic.20250122T095640Z.h5")

# get variable names 
vars = read.csv("filesToStack00200/NEON.D07.GRSM.DP4.00200.001.variables.20250123T000115Z.csv")
...
# unzip files 
for(file in filelist23){
  
  unzip(paste("filesToStack00200/",file, sep=""))
}

# stack files for fluxCO2 data 
fluxco2 <- stackEddy(filepath = "filesToStack00200/",
                     var = "fluxCo2") 

# save original 
saveRDS(fluxco2, "GRSM_eddy_covariance_fluxco2.rds")

# summarize by daily values 
fluxco2_daily <- fluxco2$GRSM %>% 
...
# save file 
write.csv(gcc_co2, "GRSM_daily_GCC90_fluxCO2.csv")

### if you need to read it in again
gcc_co2 <- read.csv("GRSM_daily_GCC90_fluxCO2.csv")

# plot 
ggplot(gcc_co2, aes(x=fluxco2.turb.flux, y=gcc_90)) + geom_point()
...
gcc23_plot <- ggplot(gcc_co2_23, aes(x=date, y=gcc_90)) + geom_line() 

co2_flux_plot <- ggplot(gcc_co2_23, aes(x=date, y=fluxco2.nsae.flux)) + geom_point() 
